...
function validate(values) {
  if (!Number.isFinite(values.initial) || values.initial < 0) return "Current savings must be zero or more.";
  if (!Number.isFinite(values.monthly) || values.monthly < 0) return "Monthly contribution must be zero or more.";
  if (!Number.isFinite(values.rate) || values.rate < 0 || values.rate > 100) return "Annual return must be between 0% and 100%.";
  if (!Number.isFinite(values.years) || values.years < 1 || values.years > 80) return "Saving period must be between 1 and 80 years.";
  if (values.goal !== null && (!Number.isFinite(values.goal) || values.goal <= 0)) return "Savings goal must be greater than zero.";
  if (values.initial === 0 && values.monthly === 0) return "Add current savings or a monthly contribution to calculate growth.";
  return "";
}
...
function calculateProjection(values) {
  ...
  return { future, contributions, interest, months };
}
...
function renderSummary(values, projection) {
  ...
  const contributionShare = projection.future > 0 ? (projection.contributions / projection.future) * 100 : 100;
...
function renderGoal(values, projection) {
  if (!values.goal) { dom.goalProgressWrap.hidden = true; return; }
...
function renderTimeline(values) {
  ...
  const yearsToShow = [];
  for (let year = step; year < values.years; year += step) yearsToShow.push(year);
  if (!yearsToShow.includes(values.years)) yearsToShow.push(values.years);
...
function calculate({ scroll = false } = {}) {
  ...
  if (error) { dom.error.textContent = error; dom.error.hidden = false; return; }
  dom.error.hidden = true;
  const projection = calculateProjection(values);
  render(values, projection);
  if (scroll) dom.results.scrollIntoView({ behavior: "smooth", block: "start" });
}

function resetForm() {
  Object.entries(state.defaults).forEach(([key, value]) => { dom[key].value = value; });
  calculate();
...
dom.form.addEventListener("submit", (event) => { event.preventDefault(); calculate({ scroll: true }); });
...
[dom.initial, dom.monthly, dom.rate, dom.years, dom.goal].forEach((input) => {
  input.addEventListener("input", () => { if (!dom.error.hidden) dom.error.hidden = true; });
});
calculate();
